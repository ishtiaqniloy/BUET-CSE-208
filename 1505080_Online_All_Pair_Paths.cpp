...
public:
    ...
    void numberOfPaths(){
        ...
        //int weightMatrix[nVertices+1][nVertices+1];
        //int parentMatrix[nVertices+1][nVertices+1];
        int counterMatrix[nVertices+1][nVertices+1];

        for(int i=1; i<=nVertices; i++){    ///O(n^2)
            for(int j=1; j<=nVertices; j++){    ///initialization
                //weightMatrix[i][j] = INFINITY;
                //parentMatrix[i][j] = NULL_VALUE;
                counterMatrix[i][j] = 0;
            }
        ...
        for(int k = 1; k<=nVertices; k++){      ///O(n^3)
            for(int i = 1; i<=nVertices; i++){
                for(int j = 1; j<=nVertices; j++){
                    ...
                    //if(counterMatrix[i][j] < counterMatrix[i][k]*counterMatrix[k][j])
                        counterMatrix[i][j] += counterMatrix[i][k]*counterMatrix[k][j];
                }
            }
