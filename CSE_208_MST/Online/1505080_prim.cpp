...
public:
    ...
    void primMST();
    void primMSTNew(int Nu, int Nv, int Nw);
...
void Graph::primMST(){
    ...
    void Graph::primMSTNew(int Nu, int Nv, int Nw){
    MinHeap heap;

    int *arr = new int[nVertices];
    int *par = new int[nVertices];
    int *child = new int[nVertices];
    ...
    for(int i=0; i<nVertices; i++){
        if(par[i]!=-1){
            printf("%d %d\n", i+1, par[i]+1);
            MST.addEdge(i, par[i], edgeWeight(i,par[i]));
        }
    ...
    while(v!=-1){
        ...
        if(v==Nv){
            printf("CYCLE FOUND, max weight= %d\n", maxWeight);
            flag = true;
            break;
        }
        u = v;
    ...
    if(maxWeight>Nw){
        printf("We have to add the new edge\n");

        MST.addEdge(Nu,Nv,Nw);
        MST.removeEdge(maxU, MaxV);
        totalWeight = totalWeight - maxWeight + Nw;
